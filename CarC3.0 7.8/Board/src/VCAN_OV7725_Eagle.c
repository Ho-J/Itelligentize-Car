...
void ov7725_eagle_port_init()
{
    ...
    dma_portx2buff_init(CAMERA_DMA_CH, (void *)&PTC_B1_IN, (void *)ov7725_eagle_img_buff, PTD1, DMA_BYTE1, CAMERA_DMA_NUM, DADDR_KEEPON);

    DMA_DIS(CAMERA_DMA_CH);				            //禁止通道硬件 DMA 请求
    disable_irq(PORTD_IRQn);                        //关闭PTA的中断
    DMA_IRQ_CLEAN(CAMERA_DMA_CH);                   //清除通道传输中断标志位
    DMA_IRQ_EN(CAMERA_DMA_CH);				        //允许 DMA 通道传输完成中断

    port_init(PTD1, ALT1 | DMA_FALLING | PULLUP );         //PCLK

    port_init(PTD0, ALT1 | IRQ_RISING | PULLUP | PF);     //场中断，上拉，上降沿触发中断，带滤波
...
void ov7725_eagle_vsync(void)
{
  ...
  if(ov7725_eagle_img_flag == IMG_START)                   //需要开始采集图像
  {
    ...
    DMA_EN(CAMERA_DMA_CH);                  //使能通道CHn 硬件请求(解决右边的图像有一部分跑到左边)
    PORTD_ISFR = 1 <<  PT1;                             //清空PCLK标志位
...
void ov7725_eagle_get_img()
{
    ...
    enable_irq(PORTD_IRQn);                         //允许PTD的中断
...
reg_s ov7725_eagle_reg[] =
{
    //寄存器，寄存器值次
    //{OV7725_COM4         , 0x41},                       //
    //{OV7725_CLKRC        , 0x00},		//调节帧数			75帧 13.3 ms一帧
    {OV7725_COM4         , 0xC1},                       //                      
    {OV7725_CLKRC        , 0x00},		//调节帧数			
    {OV7725_COM2         , 0x03},
    {OV7725_COM3         , 0xD0},
    {OV7725_COM7         , 0x40},
    {OV7725_HSTART       , 0x3F},
